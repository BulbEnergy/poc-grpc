import {
  ...
  Tariff, TariffServiceDefinition,
 ...
 */
const tariffServiceImpl: TariffServiceServiceImplementation = {
  ...
  async getTariff(
    ...
    if (!requestedTariff) {
      throw new ServerError(Status.NOT_FOUND, `cannot find ${request.tariffId}`);
    }
  ...
  async updateTariff(
    ...
    if (!Object.keys(TARIFFS).includes(request.tariff.tariffId)) {
      throw new ServerError(Status.NOT_FOUND, `Cannot find ${request.tariff.tariffId}`)
    }

    if (request.updateMask) {
      throw new ServerError(Status.UNIMPLEMENTED, "There isn't a way to use field masks with nice-proto")
    }
